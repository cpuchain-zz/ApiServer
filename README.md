...
You need to run CPUchain Core on backend to start.

Run `git clone -b 0.16 github.com/cpuchain/cpuchain` which is modified for indexing accounts and balances.

Compile, install, and then run `cpuchaind` with `-txindex -addressindex -timestampindex -spentindex`

The node will occupy more disk size than normal run.
...
Then run 'python app.py` to start, we recommend to run nginx as a reverse proxy for production environment.
